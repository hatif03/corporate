...
- `backend/app/services/knowledge_base.py` — `department_kb_text(org_id, department_id, static_fallback)`: an org's uploaded documents for a department (`orgs/{orgId}/departments/{departmentId}/knowledge_base/{docId}`), joined verbatim, falling back to that department's static embedded corpus when nothing's been uploaded. `hr_people_ops` and `customer_support` both route through this. See ADR-0016.
...
**Per-department access control** (ADR-0016): `Integration.connected_departments` — empty means unrestricted (every integration's default), non-empty is a strict allowlist. A department not on the list gets `IntegrationAccessDenied` (a `ValueError` subclass, so existing fail-soft `except ValueError:` call sites need no changes) and a standing `orgs/{orgId}/access_requests/{id}` doc is filed for an owner to resolve via `POST /api/org/{org_id}/access_requests/{id}/resolve` — a governance-timescale decision, not a per-task block. The frontend's Settings tab has a "Connected apps" panel for managing this and a pending-requests panel for resolving it.

## Sub-agent spawning and sandboxed multi-tool orchestration (ADR-0016)

- `spawn_subagent_tool` (`app/adk_agents/tools/universal.py`) lets the CEO and specific departments (`engineering_sre`, `product_analytics`) delegate a self-contained sub-task to a fresh ephemeral worker and get its real result back inline, via `workers.spawn_worker_and_await` (a synchronous, 120s-timeout sibling of the existing `spawn_worker`). It is added to `_CEO_TOOLS`/specific departments' `extra_tools` only — **never** to `_DEPARTMENT_UNIVERSAL_TOOLS`, which the ephemeral worker agents themselves use, so a spawned worker structurally cannot spawn another one.
- `execute_python_tool` (same file, wired to the CEO and `product_analytics`) runs a snippet in `app/services/sandbox.py` → a real subprocess (`python -m app.services.sandbox_runner`) — the actual isolation boundary, since only a real OS process can be force-killed regardless of what's running inside it. `RestrictedPython.compile_restricted` runs inside that subprocess as defense-in-depth on top. The child never imports `app.services.store` or holds any credential; a whitelisted `call_tool(name, **kwargs)` (read-only: `list_tasks_tool`, `list_agents_tool`, `search_memory_tool`, `read_memory`) is serviced by the parent over line-delimited JSON on stdio.
