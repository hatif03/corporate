...
### A note on inspiration and originality

Full third-party attribution — every one of them — lives in [`THIRD_PARTY_SKILLS.md`](THIRD_PARTY_SKILLS.md). The headline one worth stating plainly here: an MIT-licensed office-floor-style AI-agent app was an early visual and structural reference point for Corporate's Command Center UI, looked up directly from its live repo rather than assumed from memory. From that starting point, I built and substantially reworked Corporate's own design system, components, and layout to fit its own data model and tab set (Ask-me, Triggers, Workers, Knowledge, Board, Graph — none of which the reference has) — no code from that reference is reused as-is in the shipped app. Everything else in the app is my own: the office floor's tileset is Kenney's CC0 "1-Bit Pack", rearranged into my own room-and-corridor layout, not the reference's own separately-licensed art; every agent's name, persona, dialogue, and the "Corporate" name and branding are original, with no resemblance to the reference's own theming. In the interest of full honesty rather than a blanket "nothing but inspiration" claim: a couple of narrow backend patterns — the agent-loop doom-loop guard and session-compaction design in `runtime.py`/`compaction.py` — are genuinely adapted from a real open-source coding agent's own implementation, and a handful of department prompt sections include short excerpts adapted from other MIT-licensed domain-skill repos. Both are small, both are fully attributed by source, author, and license in `THIRD_PARTY_SKILLS.md`, and neither is core application logic — the department contract, the verification pipeline, the messaging fabric, and every department's actual pipeline are original.
...
- **Source**: https://github.com/hatif03/corporate (public — no separate access grant needed for judging)
- **Demo video**: [ADD THE YOUTUBE/VIMEO LINK HERE ONCE UPLOADED] — the hackathon's submission form requires the video hosted on YouTube or Vimeo specifically, not just the mp4 in this repo; the same clip already lives at `frontend/public/media/corporate-demo.mp4` and is embedded on the live app's landing page and in `README.md`.
...
## Track fit and an honest read on my chances

**Why Fortified Enterprise Fleet**: the track's own rules ask for a network of institutional agents "cataloged for cross-department use," that "safely maintain context across weeks of asynchronous operations," and that "interact with production data without violating enterprise compliance, data sovereignty, or security policies" — illustrated with four example component categories (Discovery & Lifecycle, Core Execution & State, Security & Governance, Telemetry) that are explicitly guidance, not a checklist of named products every submission must integrate. Mapped honestly against what this project actually has, not what would look good on paper:

| Track's example component | What Corporate actually has |
|---|---|
| Agent Registry (cataloging enterprise-approved agents) | The `DepartmentSpec` contract + the `new-department` scaffolding skill — every department is a discoverable, versioned Python package following one shape, not a bespoke one-off |
| Agent Runtime (long-running async execution) | Cloud Run + Pub/Sub push dispatch, plus ephemeral one-off workers for out-of-band async jobs |
| Memory Bank (persistent cross-session context) | Firestore-backed ADK sessions (survives Cloud Run's ephemeral, scale-to-zero instances) + real semantic memory (Vertex AI embeddings, gated auto-surfacing) |
| Agent Identity (zero-trust access control) | Firebase Auth + independent backend org-membership checks, defense-in-depth (two layers, neither sufficient alone) |
| Agent Gateway (unified routing and policy enforcement) | The integration broker — the only code path that ever dereferences a real credential, with per-department access-control allowlisting and a standing approval queue |
| Model Armor (prompt injection / tool poisoning / PII guardrails) | Partial, honestly: PII redaction before any LLM call on user-generated content, a real OS-subprocess sandbox with `RestrictedPython` for the one tool that executes model-authored code, and a hard hop-cap against runaway agent loops — but no single dedicated prompt-injection-detection layer sitting in front of every tool call. The clearest gap in this table, not glossed over. |
| Agent Observability (OpenTelemetry-style audit logs + reasoning traces) | A tamper-evident, hash-chained audit log with a live integrity-verification badge, per-agent reasoning traces on every Command Center terminal, and per-aspect verification votes — not OpenTelemetry-instrumented specifically, but the same intent: a real, checkable record of what every agent did and why |

**Against the actual judging language** (not just the category names): Innovation & Operational Utility (40%) asks, for this track specifically, "is the task complex enough to warrant a multi-agent system? Does the system intelligently delegate tasks to specialized sub-agents?" — the CEO's `create_task` delegation, routed over real Pub/Sub to whichever of nine genuinely different department pipelines fits, is exactly that, not a single agent role-playing nine personas. It also asks whether the team "built this for an 'Unlikely Hero' outside of standard corporate roles" — my honest answer is no: this is squarely aimed at the standard department roster (Finance, Engineering, Legal, Sales, HR, Support, Marketing, Analytics) a real company already has, not a novel outsider role, and I'd rather say that plainly than stretch a claim. Architectural Discipline & Tech Stack (30%) — 20 ADRs, one department contract every department follows identically, defense-in-depth auth, and nine separate real bugs caught and fixed by live-testing in the final 72 hours (see Challenges, above) that would otherwise have shipped broken, non-compliant, or both. Demo & Production Readiness (30%) — actually deployed and live on Cloud Run/Firebase, a clean architecture diagram, reproducible setup instructions, and the demo video's own Cloud Console segment as direct visual proof of Google Cloud deployment, not just an assertion.

**Real weaknesses, not glossed over**: no dedicated Model Armor-style guardrail layer (above); no live user load ever tested against this (a hackathon demo, not a production SLA); a small, fast-moving build with corners genuinely deferred (`SequentialAgent`, in-process worker execution) and documented as such rather than hidden; some of the observability work (per-aspect votes, worker trace, the audit badge) landed in the final 72 hours, which is a real timing risk even though it's tested and live; and I made no attempt at the "Unlikely Hero" angle the track's judging notes specifically call out as a positive signal.
