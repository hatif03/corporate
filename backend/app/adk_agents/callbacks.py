...
async def before_agent_callback(context: Context):
    org_id, agent_id = _ids(context)
    store.update_agent_status(org_id, agent_id, AgentStatus.THINKING, action="reasoning")
    return None


async def before_tool_callback(tool: BaseTool, args: dict[str, Any], context: Context):
    org_id, agent_id = _ids(context)
...
async def after_tool_callback(tool: BaseTool, args: dict[str, Any], context: Context, result: dict[str, Any]):
    org_id, agent_id = _ids(context)
    store.append_trace(org_id, agent_id, f"{tool.name}({args}) -> {result}", kind="tool")
    store.update_agent_status(org_id, agent_id, AgentStatus.WORKING, carrying=CarryingToken.NONE)
    return None


async def after_agent_callback(context: Context):
    org_id, agent_id = _ids(context)
